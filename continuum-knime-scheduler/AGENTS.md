...
- `WorkflowScheduleApiClient` and `KnimeScheduleWorkflowMapper` build/read `continuum-commons`'s canonical `ContinuumWorkflowModel` directly (this module depends on `continuum-commons`) rather than a local duplicate — a prior local-duplicate DTO set was removed because `RestTemplate`'s default Jackson config has `FAIL_ON_UNKNOWN_PROPERTIES` enabled, so deserializing api-server's actual (full-shaped) response into a slimmer local type threw `UnrecognizedPropertyException`. This pulls Temporal/Avro/Parquet/Hadoop transitively into this module's classpath and container image — an accepted tradeoff for schema correctness. `KnimeScheduleWorkflowMapper` still duplicates `KNIMEWorkflowExecutorNodeModel`'s node-model class name and `propertiesSchema` from the sibling `continuum-feature-knime` repo — keep both in sync if that node's contract changes.
